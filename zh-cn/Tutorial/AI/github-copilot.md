...
**Q: 登录时出现以下问题**
- 无法在浏览器中打开 github 登录页面
- 浏览器中登录成功，但插件长时间无反应
- 浏览器中登录成功，但插件显示 **已登录但未授权**
- 浏览器中登录成功，但插件显示 **Device flow GitHub authentication failed:: HttpError: other side closed**

A: 这通常由以下几种原因造成：
1.  **网络问题**: 这是最常见的原因。插件无法连接到 GitHub 的授权服务器（需要科学上网）。请检查您的网络连接、防火墙或代理设置，确保可以正常访问 `https://github.com`。日志中可能会出现 "无法访问 https://github.com/" 或 "request timed out" 等错误，遇到此类问题时可尝试切换到一个比较稳定的代理试试，或者将代理模式改为全局模式或者虚拟网卡模式后重试。
