...
|--project		|必填，項目名稱															|
|--appid		|如果上傳發行小程序到微信平臺，則必填微信小程序appid												|
...
|--sourceMap    |生成SourceMap,值爲 true 時生效，默認爲 false				|
|--robot		|指定微信ci機器人編號（取值範圍：1 ~ 30），默認爲 1						|

注意：`--robot`參數，僅支持HBuilderX 3.6.18+版本。同時需要升級 [微信小程序上傳CI插件](https://ext.dcloud.net.cn/plugin?id=7199)
...
# 編譯uni-app項目到微信小程序，並上傳發行小程序到微信平臺
...
cli publish --platform mp-weixin --project 項目名稱 --upload true --appid 小程序appid --description 發佈描述 --version 發佈版本 --privatekey 小程序上傳密鑰文件  --robot 2
```


## 小程序代碼上傳密鑰@uploadPrivateKey

> HBuilderX 3.3.7+, uni-app 發行到微信小程序，支持自動上傳代碼到微信平臺，無需再通過微信開發者工具上傳發行。[詳情](/Tutorial/App/uni-app-publish-mp-weixin)
>
> HBuilderX 發行微信小程序，需要提供微信小程序代碼上傳密鑰

通過微信小程序CI，使用上傳密鑰上傳代碼，無需打開微信開發者工具，一鍵完成微信小程序代碼的上傳、預覽等操作。
...
1. 請正確填寫`微信小程序appid`和`privatekey`
2. 如果開啓了`IP白名單`，請確保微信平臺已配置 IP 白名單
