> 仅适用于HBuilderX 3.1.5+版本

## cli pack打包

cli打包使用流程
1.打开HBuilder X目录,命令行输入cli.exe open 打开HBuilder X 或 手动点击打开HBuilder X
2.导入要打包的项目,命令行输入cli.exe project open --path 项目路径
3.命令行输入打开命令cli.exe pack --参数名字 参数值 打包详细参数值说明详见cli.exe pack支持的参数,打包过程中如果有错误会给出相应的错误信息并中断操作，打包成功后传统打包会输出打包成功的下载地址，安心打包会输出打包成功后的路径。

注:cli.exe 在macOS系统上的文件名为cli
